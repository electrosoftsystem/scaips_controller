...
const FeedArea = ({ refreshTrigger, onRefreshReady, isOwner = false }) => {
  ...
  // Fetch posts on load or refresh trigger
  useEffect(() => {
    if (isAuthenticated) {
      fetchMyPosts();
    } else {
  ...
  const fetchMyPosts = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiService.getMyPosts({ limit: 50 });
  ...
  const handleBookmark = async (postId) => {
    try {
      // Optional backend call if supported
  ...
  const renderPost = (post) => {
    const author = post.author?.user || post.author || {};
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm overflow-hidden relative bg-gradient-to-br from-blue-500 to-purple-600">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-200">
          <div className="flex items-center gap-6">
            ...
            >
              ...
              />
              <span>{Number(post.reaction_count) || 0}</span>
            </button>
